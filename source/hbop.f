...
C add a condition to avoid extrapolation beyond next bf-edge
C  Bertrand Plez  2025/08/25
      IF (NLO.EQ.1 .and. w.lt.L2(N(2))) CALL LOGINT(L1,G1,N(1),W,G)
      IF (NLO.EQ.2 .and. w.lt.L3(N(3))) CALL LOGINT(L2,G2,N(2),W,G)
      IF (NLO.EQ.3 .and. w.lt.L4(N(4))) CALL LOGINT(L3,G3,N(3),W,G)
      IF (NLO.EQ.4 .and. w.lt.L5(N(5))) CALL LOGINT(L4,G4,N(4),W,G)
      IF (NLO.EQ.5 .and. w.lt.L6(N(6))) CALL LOGINT(L5,G5,N(5),W,G)
      IF (NLO.EQ.6 .and. w.lt.L7(N(7))) CALL LOGINT(L6,G6,N(6),W,G)
      IF (NLO.EQ.7 .and. w.lt.L8(N(8))) CALL LOGINT(L7,G7,N(7),W,G)
      IF (NLO.EQ.8 .and. w.lt.L9(N(9))) CALL LOGINT(L8,G8,N(8),W,G)
      IF (NLO.EQ.9 .and. w.lt.L10(N(10))) CALL LOGINT(L9,G9,N(9),W,G)
      IF (NLO.EQ.10 .and. w.lt.L11(N(11)))
     &   CALL LOGINT(L10,G10,N(10),W,G)
      IF (NLO.EQ.11 .and. w.lt.L12(N(12)))
     &   CALL LOGINT(L11,G11,N(11),W,G)
      IF (NLO.EQ.12 .and. w.lt.L13(N(13)))
     &   CALL LOGINT(L12,G12,N(12),W,G)
      IF (NLO.EQ.13 .and. w.lt.L14(N(14)))
     &   CALL LOGINT(L13,G13,N(13),W,G)
      IF (NLO.EQ.14 .and. w.lt.L15(N(15)))
     &   CALL LOGINT(L14,G14,N(14),W,G)
C No fix for the last one. This will presumably never be used for too
C large wavelengths.
