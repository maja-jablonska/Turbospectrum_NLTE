...
def _synthesis_task(batch):
    ...
    for global_index, row_values in batch:
        ...
        if is_intensity and str(mu_sampling.get("mode", "none")).strip().lower() in {"", "none"}:
            # Regular grids supply a per-row target_mu from the mu axis;
            # use "nearest" so the closest mu point is selected within the
            # requested range.  Fall back to "random" only when no target is
            # available (legacy / non-regular-grid behaviour).
        ...
        if os.path.exists(spec_path):
            try:
                ...
                if is_intensity:
                    ...
                    # to column-index estimation when the header is absent.
