...
def _failed_row_examples(statuses: Sequence[str], messages: Sequence[str], *, limit: int = 5) -> List[str]:
    examples: List[str] = []
    for idx, (status, message) in enumerate(zip(statuses, messages)):
        status_text = str(status)
        if status_text in {"success", "skipped"}:
            continue
        msg = str(message).strip() or "<no message>"
        examples.append(f"row={idx} status={status_text} message={msg}")
...
def _validate_synthesis_results(
    ...
    usable_rows = int(status_counts.get("success", 0)) + int(status_counts.get("skipped", 0))
    failed_rows = len(statuses) - usable_rows
    failure_examples = _failed_row_examples(statuses, messages)

    if failed_rows > 0:
        detail = f"; examples: {' | '.join(failure_examples)}" if failure_examples else ""
        logger.warning(
            "Synthesis finished with %d failed rows and %d usable rows%s",
            failed_rows,
            usable_rows,
            detail,
        )
    ...
    if usable_rows == 0:
        detail = f" Examples: {' | '.join(failure_examples)}" if failure_examples else ""
        raise RuntimeError(
            f"Synthesis produced no usable spectra; status_counts={status_counts}.{detail}"
        )

    finite_flux_rows = 0
    if fluxes.ndim == 2 and fluxes.size:
        finite_flux_rows = int(np.count_nonzero(np.any(np.isfinite(fluxes), axis=1)))
    if finite_flux_rows == 0:
        detail = f" Examples: {' | '.join(failure_examples)}" if failure_examples else ""
        raise RuntimeError(
            "Synthesis completed without any finite flux rows; "
            f"status_counts={status_counts}.{detail}"
        )

    if finite_flux_rows < usable_rows:
        logger.warning(
            "Only %d/%d usable rows contain finite flux values",
            finite_flux_rows,
            usable_rows,
        )
...
def main() -> None:
    ...
    status_counts = _validate_synthesis_results(statuses, messages, fluxes, logger)
