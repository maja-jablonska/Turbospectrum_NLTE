...
def _verify_continuum_saved(spectra_zarr: str) -> None:
    ...
    raw_status_counts = root.attrs.get("status_counts", {}) or {}
    if isinstance(raw_status_counts, dict):
        success_count = int(raw_status_counts.get("success", 0) or 0)
        skipped_count = int(raw_status_counts.get("skipped", 0) or 0)
        if success_count + skipped_count <= 0:
            raise ValueError(
                f"Synthesis output {path} contains no usable spectra; "
                f"status_counts={dict(raw_status_counts)}"
            )
