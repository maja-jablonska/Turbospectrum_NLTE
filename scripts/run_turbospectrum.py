...
"""
            ...
            if not _file_has_content(current_result_file):
                if os.path.exists(current_result_file):
                    try:
                        size_bytes = os.path.getsize(current_result_file)
                    except OSError:
                        size_bytes = -1
                    return build_result(
                        "error",
                        f"bsyn produced empty output ({mode_str}, size={size_bytes} bytes): {current_result_file}",
                    )
                return build_result(
                    "error",
                    f"bsyn produced no output ({mode_str}): {current_result_file}",
                )
